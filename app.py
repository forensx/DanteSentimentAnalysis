...
from annotations_helper import get_annotations
...
def update_graph_single(section, df, annotations):
    ...
    hover_code = ["{}: <br>Canto #{}".format(
        section, x) for x in range(1, len(df['Canto']))]
...
def update_graph_all():
    ...
    purgatorio_hover = [
        "Purgatorio: <br>Canto #{}".format(x) for x in range(1, 34)]
    paradiso_hover = ["Paradiso: <br>Canto #{}".format(
        x) for x in range(1, 34)]
...
def update_graph_average():
    ...
    mean_sent = [avg_sentiment_inferno,
                 avg_sentiment_purgatorio, avg_sentiment_paradiso]
    mean_first_deriv = [avg_first_inferno,
                        avg_first_purgatorio, avg_first_paradiso]
    mean_second_deriv = [avg_second_inferno,
                         avg_second_purgatorio, avg_second_paradiso]
    ...
    dcc.Tabs(id='circos-control-tabs', value='Canto', children=[
        dcc.Tab(
            ...
            children=html.Div(
                ...
                children=[
                    html.H4(
                        'Select a canto by clicking on the points on the graph!')
                ],
            ),
        ),
        dcc.Tab(
            ...
            children=html.Div(
                id='control-tab',
                children=[
                    html.H4('Lit HL - English Project', style={
                            'font-size': '24pt', 'font-wight': '200', 'letter-spacing': '1px'}),
                    html.Div(
...
def update_graph(value):
    ...
    annotations = get_annotations(value)
    return update_graph_single(value, df, annotations)
...
def update_canto(selection, section):
    ...
    return html.Div(
        children=[
            html.H3(section + ' - Canto #' +
                    str(cantoNumber), style={"fontSize": 18}),
            html.P("Sentiment for Canto = " +
                   str(sentiment), style={'fontSize': 16}),
